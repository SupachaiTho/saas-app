class ProjectsController < ApplicationController
  ...
  def add_user
    @project_users = UserProject.new(user_id: params[:user_id], project_id: @project.id)
    
    respond_to do |format|
      if @project_users.save
  ...
  private
    ...
    def project_params
      params.require(:project).permit(:title, :details, :expected_completaion_date, :tenant_id)
    end
    ...
    def verify_tenant
      unless params[:tenant_id] == Tenant.current_tenant_id.to_s
        redirect_to :root, flash: {error: 'You are not authorized to access ant organizational other than your own'}
      end
    end
end
